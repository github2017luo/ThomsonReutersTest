...
import saveTrialAndChildSobjectsAddUser from '@salesforce/apex/OmniTrialsAddUserController.saveTrialAndChildSobjectsAddUser';
...
import getExistingTrailDetailsAddUser from '@salesforce/apex/OmniTrialsAddUserController.getExistingTrailDetailsAddUser';
...
import {omniTrialBlacklistAlertLabel,omniTrialExpiredAlert,omniTrialCompetitorAlertLabel, omniTrialDomainBlacklistAlertLabel,OmniTrialLapsedAccountAlertLabel,
...
import {formatDate, digitalCheckStatusBlocked, digitalCompetitorCheck} from './omniAddUserHelper';
...
export default class LightningDatatableExample extends NavigationMixin(LightningElement) {
    @api recordId; //Trial Record ID
    @api trialOptyId;//Trial Oppty ID
    @api objectApiName; 
    ...
    connectedCallback(){
        ...
        this.optyLink = '/' + this.trialOptyId;
        
        let optyPromise = fetchOpportunity({recordId : this.trialOptyId})
            .then(result => {
        ...
        optyPromise.then(()=>{
            ...
            try{
                //Catching Error If Null
                this.ssdId = this.optySobject.Source_System_Detail__c;
                if(this.ssdId != "undefined" && this.ssdId != null){
                    this.ssdIdLink = '/' + this.ssdId;
                    this.ssdName = this.optySobject.Source_System_Detail__r.Name;
                }
            }catch(err){
               
            }
            //this.accSanctionedStatus = this.optySobject.Account.Sanctioned_Status__c;
            //this.optyStage = this.optySobject.StageName;
                    ...
                    if(!this.isErrorInInitialCheck)
                    {
                        ...
                        gsiContactPromises.then(() => {
                            onlineContactPromises.then(() => {
                                ...
                                var entTrialsWrap = [];
                                let entTrialPromise = getExistingTrailDetailsAddUser({trialId : this.recordId, trialContIds : null})
                                                    .then(result=>{
                                                        entTrialsWrap = JSON.parse(JSON.stringify(result));
                                                        console.log("@@Wrapper "+entTrialsWrap);
                                                    })
                                                    .catch(error =>{
                                                    console.log(JSON.stringify(error));
                                                    const errorMsg = reduceErrors(error);
                                                    this.dispatchEvent(showToast('error','sticky','Suc!','Error while fetching existing Trial Details.'));
                                                    })
                                entTrialPromise.then(()=>{
                                    //Blocking to add users if trial status is Expired or Ended
                                    if(entTrialsWrap.entTrial.Trial_Status__c === 'Expired' || entTrialsWrap.entTrial.Trial_Status__c === 'Ended'){
                                        this.dispatchEvent(showToast('error','sticky','Error!',omniTrialExpiredAlert));
                                        this.closeQuickAction();
                                        this.isErrorInInitialCheck  = true;
                                    }
                                   else if(entTrialsWrap != null && entTrialsWrap != undefined){
                                      ...
                                      //this.trailContactsSelected = entTrialsWrap.trialContacts;
                                        ...
                                        //this.selectedRecord.push(entTrialsWrap.primaryContactEmail);  Removing 
   ...
   /*getPrimaryContact(event){
    ...
    goToReviewPage(event){
       ...
       //GSI Contact 
        ...
        else
        {
           ...
           //alert('Selected Contacts'+this.trailContactsSelected);
            ...
            olcOnepasspromise.then(()=>{
                console.log(JSON.stringify(trialContsUpdated));
    ...
    saveTrials(event)
    {
        ...
        //this.trialRecord.Contact_Name__c = this.primaryContactId;
        this.trialRecord.Opportunity__c = this.trialOptyId;
        if(this.trailContactsSelected){
            ...
            for (var i = 0; i < this.trailContactsSelected.length; i++) {  
                ...
                tempRecord.requestType = 'TRIAL_ADDUSER';
        ...
        var entTrialsWrap = [];
        let savePromise = saveTrialAndChildSobjectsAddUser({trialSobj : this.trialRecord,selectedProducts :JSON.stringify(this.seletedProductList), selectedContacts :JSON.stringify(this.trailContactsSelected)})
                            .then(result =>{
                                this.error = undefined;
        ...
        savePromise.then(()=>{
            if(entTrialsWrap != null && entTrialsWrap != undefined)
            {
                ...
                console.log('trialContIds: '+trialContIds);
                console.log('trialPrdIds: '+trialPrdIds);
                ...
                eloqua.then(()=>{
                    ...
                    if(eloquaResult === true){
                        this.dispatchEvent(showToast('success','sticky','Success!','Trial Request submitted sucessfully.'));
